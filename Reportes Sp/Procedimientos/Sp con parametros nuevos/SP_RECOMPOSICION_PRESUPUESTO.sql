...
AS
	...
	DECLARE @QUERY3 NVARCHAR(MAX) = N'
		DECLARE CURSOR3 CURSOR FOR
			SELECT (A.CODI_ESM), 
				(SELECT E.MPRVAL FROM AUDI_ESM E WHERE E.OPNCOD = @PRE_ING AND E.VIGENCIA = @IN_VIGENCIA AND E.CODI_ESM = A.CODI_ESM) PRESU_INICIAL
			FROM AUDI_ESM A
			WHERE A.OPNCOD IN (@RED_ING ,@ADI_ING)
				AND A.FEC_AUTORI BETWEEN @IN_FECHA_INI AND @IN_FECHA_FIN
					AND A.VIGENCIA = @IN_VIGENCIA
	...
	DECLARE @QUERY4 NVARCHAR(MAX) = N'
		DECLARE CURSOR4 CURSOR FOR
			SELECT A.MPRDOCN, A.CODI_ESM,
				ISNULL((SELECT E.MPRVAL FROM AUDI_ESM E WHERE E.OPNCOD=@ADI_ING AND E.VIGENCIA = @IN_VIGENCIA AND E.CODI_ESM = A.CODI_ESM AND E.MPRDOCN = A.MPRDOCN),
						(SELECT E.MPRVAL FROM AUDI_ESM E WHERE E.OPNCOD=@RED_ING AND E.VIGENCIA = @IN_VIGENCIA AND E.CODI_ESM = A.CODI_ESM AND E.MPRDOCN = A.MPRDOCN)) VALOR,
				A.FEC_AUTORI
			FROM AUDI_ESM A
			WHERE A.OPNCOD IN (@RED_ING ,@ADI_ING)
				AND A.FEC_AUTORI BETWEEN @IN_FECHA_INI AND @IN_FECHA_FIN
