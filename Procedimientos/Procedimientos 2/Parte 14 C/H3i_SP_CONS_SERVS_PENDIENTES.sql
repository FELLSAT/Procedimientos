...
BEGIN
    ...
    DELETE FROM tt_RESULTADO_3;
    UTILS.IDENTITY_RESET('tt_RESULTADO_3');
   
    INSERT INTO tt_RESULTADO_3 ( 
        SELECT NVL(REG.NU_NUME_REG, 0) NU_NUME_REG  ,
            SER.NU_OPCI_SER TIPOSERVICIO  ,
            PAC.NU_HIST_PAC ,
            HPR.NU_NUME_HPRO ,
            MCA.NU_NUME_MOVI ,
            LAB.NU_NUME_LABO ,
            ...
            SER.NO_NOMB_SER ,
            HPR.NU_CANT_HPRO ,
            MEDORDENA.CD_CODI_MED CODIGOMEDICOORDENA  ,
            MEDORDENA.NO_NOMB_MED MEDICOORDENA  ,
            ESPORDENA.CD_CODI_ESP CODIGOESPECIALIDADORDENA  ,
            ESPORDENA.NO_NOMB_ESP ESPECIALIDADORDENA  ,
            CASE 
            ...
            NVL(ESP.NO_NOMB_ESP, ProcedimientoUnicaEspe(SER.CD_CODI_SER, 3)) ESPECIALIDADREALIZANOMBRE  ,---FUNCION
            NVL(MED.CD_CODI_MED, ' ') CODIGOMEDICOREALIZA  ,
            NVL(MED.NO_NOMB_MED, ' ') MEDICOREALIZA  ,
            NVL(MCA.NU_TIAT_MOVI, 1) TIPOATENCION  ,
            NVL(MCA.NU_TIPO_MOVI, 1) TIPOCARGO  ,
        ...
        INNER JOIN SERVICIOS SER   
            ...
            AND ( HPR.NU_NUME_LABO_HPRO IS NULL
            OR HPR.NU_NUME_LABO_HPRO = 0 )
        ...
        WHERE  PAC.NU_HIST_PAC LIKE v_NumHistoria
            ...
            AND SER.NU_OPCI_SER IN (  SELECT IDTIPO 
                                      FROM TABLE(F_FILTRA_TIPOSERVICIOS(v_TipoServ)))  ----FUNCION
            AND HPR.NU_ESTA_HPRO <> 2
        GROUP BY REG.NU_NUME_REG,PAC.NU_HIST_PAC,
            HPR.NU_NUME_HPRO,MCA.NU_NUME_MOVI,
            LAB.NU_NUME_LABO,SER.NU_OPCI_SER,
            PAC.NU_TIPD_PAC,PAC.NU_DOCU_PAC,
            PAC.DE_PRAP_PAC,PAC.DE_SGAP_PAC,
            PAC.NO_NOMB_PAC,PAC.NO_SGNO_PAC,
            HCL.NU_NUME_HICL,HPR.NU_NUME_HEVO_HPRO,
            HPR.NU_ORDE_HPRO,HCL.FE_FECH_HICL,
            SER.CD_CODI_SER,SER.NO_NOMB_SER,
            HPR.NU_CANT_HPRO,MEDORDENA.CD_CODI_MED,
            MEDORDENA.NO_NOMB_MED,ESPORDENA.CD_CODI_ESP,
            ESPORDENA.NO_NOMB_ESP,ESP.CD_CODI_ESP,
            ESP.NO_NOMB_ESP,MED.CD_CODI_MED,
            MED.NO_NOMB_MED,MCA.NU_TIAT_MOVI,
            MCA.NU_TIPO_MOVI,MCA.NU_NUME_CONV_MOVI,
            MCA.CD_REGIMEN_MOVI,PAC.CD_CODI_LUAT_PAC,
            HPR.DE_INDI_HPRO,HPR.NU_ESTA_HPRO );
   ------    
    OPEN  cv_1 FOR
        SELECT * 
        FROM tt_RESULTADO_3 
