CREATE OR REPLACE VIEW FACTANUL_REGPAQUDETSERV
 -- =============================================      
 -- Author: 
 -- =============================================
AS 
    ...
    RIGHT JOIN HONORARIOS 
    RIGHT JOIN LUGAR_ATENCION LUGAR_ATENCION_FACO
    RIGHT JOIN FACTURAS_ANULADAS 
    INNER JOIN REGISTRO 
    ...
    INNER JOIN CONVENIOS 
        ...
        ON R_SER_TARB.CD_CODI_TARB_RST = CONVENIOS.CD_CODI_TARB_CONV
        ON R_REGISTRO_CONV.NU_NUME_CONV_RECO = CONVENIOS.NU_NUME_CONV
        ON REGISTRO.NU_HIST_PAC_REG = PACIENTES.NU_HIST_PAC
        AND REGISTRO.NU_NUME_REG = R_REGISTRO_CONV.NU_NUME_REG_RECO
    ...
    INNER JOIN LABORATORIO 
        ...
        AND SERVICIOS.CD_CODI_SER = LABORATORIO.CD_CODI_SER_LABO
        ON FACTURAS_ANULADAS.NU_NUME_MOVI_FACA = MOVI_CARGOS.NU_NUME_MOVI
    INNER JOIN PAQUETES 
        ON FACTURAS_ANULADAS.NU_NUME_PAQU_FACA = PAQUETES.NU_NUME_PAQU
    RIGHT JOIN FACTURAS_CONTADO 
    JOIN MOVI_CARGOS MOVI_CARGOS_PAQU 
        ON FACTURAS_CONTADO.NU_NUME_FACO = MOVI_CARGOS_PAQU.NU_NUME_FACO_MOVI
        ON PAQUETES.NU_NUME_MOVI_PAQU = MOVI_CARGOS_PAQU.NU_NUME_MOVI
    ...
    WHERE (MOVI_CARGOS.NU_NUME_PAQU_MOVI > 0 )
        AND (MOVI_CARGOS.NU_NUME_REG_MOVI > 0 )
