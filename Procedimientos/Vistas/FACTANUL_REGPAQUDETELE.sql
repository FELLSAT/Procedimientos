CREATE OR REPLACE VIEW FACTANUL_REGPAQUDETELE
 -- =============================================      
...
AS 
    SELECT  PACIENTES.NU_HIST_PAC HISTORIA_PAC  ,
        ...
        MOVI_CARGOS_PAQU.FE_FECH_MOVI FECHA_HORA_MOVIMIENTO  ,
        FACTURAS_ANULADAS.NU_NUME_PAQU_FACA NUM_PAQUETE  ,
        ...
        MOVI_CARGOS_PAQU.NU_NUME_MOVI NUM_MOVIMIENTO  ,
        FACTURAS_CONTADO.VL_RECA_FACO VALOR_RECARGO_FACT  ,
        FACTURAS_CONTADO.VL_FINA_FACO VALOR_FINANCIADO_FACT  ,
        CASE  FACTURAS_ANULADAS.NU_NUME_PAQU_FACA
            WHEN 0 THEN  FACTURAS_ANULADAS.NU_NUME_MOVI_FACA
            ELSE  FACTURAS_ANULADAS.NU_NUME_PAQU_FACA
        END GRUPO_PAQUETE  ,
        LUGAR_ATENCION_FACO.DE_DIRE_LUAT DIRECCION_LUGARFACTURACION  ,
        LUGAR_ATENCION_FACO.DE_TELE_LUAT TELEFONO_LUGARFACTURACION  ,
        'DP' TIPOREG_PAQUETE  ,
        MOVI_CARGOS_PAQU.NU_TIPO_MOVI TIPO_MOVIMIENTO  ,
        ...
        MOVI_CARGOS_PAQU.NU_ORDE_MOVI ,
        MOVI_CARGOS_PAQU.CD_CODI_CECO_MOVI 
    FROM LUGAR_ATENCION LUGAR_ATENCION_FACO
    RIGHT JOIN LUGAR_ATENCION LUGAR_ATENCION_MOVI
    RIGHT JOIN FACTURAS_CONTADO 
    INNER JOIN MOVI_CARGOS MOVI_CARGOS_PAQU   
        ON FACTURAS_CONTADO.NU_NUME_FACO = MOVI_CARGOS_PAQU.NU_NUME_FACO_MOVI
    LEFT JOIN PAQUETES 
    INNER JOIN ARTICULO 
    INNER JOIN R_REGISTRO_CONV 
    ...
    INNER JOIN CONVENIOS    
        ...
        ON R_PAC_EPS.CD_NIT_EPS_RPE = EPS.CD_NIT_EPS
        ON R_REGISTRO_CONV.NU_NUME_CONV_RECO = CONVENIOS.NU_NUME_CONV
    INNER JOIN REGISTRO    
        ON PACIENTES.NU_HIST_PAC = REGISTRO.NU_HIST_PAC_REG
        AND R_REGISTRO_CONV.NU_NUME_REG_RECO = REGISTRO.NU_NUME_REG
    INNER JOIN MOVI_CARGOS MOVI_CARGOS_1   
        ON REGISTRO.NU_NUME_REG = MOVI_CARGOS_1.NU_NUME_REG_MOVI
    INNER JOIN FORMULAS    
        ON MOVI_CARGOS_1.NU_NUME_MOVI = FORMULAS.NU_NUME_MOVI_FORM
        ON ARTICULO.CD_CODI_ARTI = FORMULAS.CD_CODI_ELE_FORM
    INNER JOIN FACTURAS_ANULADAS    
        ON MOVI_CARGOS_1.NU_NUME_MOVI = FACTURAS_ANULADAS.NU_NUME_MOVI_FACA
        ON PAQUETES.NU_NUME_PAQU = FACTURAS_ANULADAS.NU_NUME_PAQU_FACA
        ON MOVI_CARGOS_PAQU.NU_NUME_MOVI = PAQUETES.NU_NUME_MOVI_PAQU
    LEFT JOIN HONORARIOS    
        ON MOVI_CARGOS_1.NU_NUME_MOVI = HONORARIOS.NU_NUME_MOVI_HONO
        ON LUGAR_ATENCION_MOVI.CD_CODI_LUAT = MOVI_CARGOS_1.CD_CODI_LUAT_MOVI
        ON LUGAR_ATENCION_FACO.CD_CODI_LUAT = FACTURAS_CONTADO.CD_CODI_LUAT_FACO
    WHERE (MOVI_CARGOS_PAQU.NU_NUME_REG_MOVI > 0)
        AND (FACTURAS_ANULADAS.NU_NUME_PAQU_FACA > 0)
        AND (FACTURAS_CONTADO.NU_ESTA_FACO = 0 );