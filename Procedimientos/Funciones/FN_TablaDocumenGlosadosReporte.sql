CREATE OR REPLACE FUNCTION FN_TablaDocumenGlosadosReporte
 -- =============================================      
...
(
	V_ESTADO NUMBER,
	V_VALOR NUMBER,
	V_FILTROFECHA NUMBER,
	V_FECHAINI DATE,
	V_FECHAFIN DATE,
	V_CODGLOSA VARCHAR2,
	V_FILTROVALOR NUMBER,
	V_OPCIONORDEN NUMBER,
	V_TIPOORDEN NUMBER
)
	RETURN TYPE_TAB_TablaDocGlosadosRepor
AS
	V_TABLA  TYPE_TAB_TablaDocGlosadosRepor := TYPE_TAB_TablaDocGlosadosRepor();
	-------------------------------------
	TYPE RECORDTYPE IS RECORD (
		...
		COL11 VARCHAR2(20), COL12 VARCHAR(20)
	);
	-------------------------------------	
	CURSOR CV_1 IS
		SELECT COD_AUDI_DOCUM_TB,
		...
		FROM(SELECT COD_AUDI_DOCUM_ARG COD_AUDI_DOCUM_TB,
			...
			WHERE ESTADO_ARG = V_ESTADO
				...
				AND VALOR_FACTURADO_ARG = CASE 
											WHEN V_FILTROVALOR = 1 THEN V_VALOR 
											ELSE VALOR_FACTURADO_ARG 
										  END
...
BEGIN
	...
	LOOP
		V_TABLA.EXTEND;
		V_TABLA(V_TABLA.LAST) := TYPE_FN_TablaDocGlosadosReport(
...
CREATE TYPE TYPE_FN_TablaDocGlosadosReport AS OBJECT (
	...
	COD_CAUSA_TIPO_AGSI VARCHAR(20)
);

--CREACION DEL TYPE COMO UNA TBLA DE ESE TIPOP
CREATE TYPE TYPE_TAB_TablaDocGlosadosRepor IS TABLE OF TYPE_FN_TablaDocGlosadosReport;
