CREATE OR REPLACE PROCEDURE H3i_SP_BUSC_PACIENTE_ANEX_UNAL
 -- =============================================      
 ...
 BEGIN
 	OPEN CV_1 FOR
	 	SELECT 20,NU_HIST_PAC,
		       ...
		       DE_OTRO_TELE_PAC, NU_HIST_PAC_PAU,
		       NU_DOCU_PAC_PAU, NU_TIPD_PAC_PAU,
		       CD_CODI_EST_PAU, CD_CODI_PA_PAU,
		       TX_ESTA_SIA_PAU, CD_CODI_DEPE_PAU,
		       TX_ESTA_SARA_PAU, CD_CODI_S_PAU,
		       TX_ESTU_PRAC_CLIN_PAU, TX_ESTU_MOVI_ACAD_PAU,
		       NU_NUME_MATR_PAU, TX_GRADO_PAU,
		       TX_TELE_PROC_PAU, TX_LOC_PAU,
		       CD_TIUS_PAU, TX_TURN_PAU,
		       NU_CARG_PAU, NU_REGI_PAU,
		       NU_DESP_PAU, DE_PRAP_RES_PAU,
		       DE_SGAP_RES_PAU, NU_NOMB_RES_PAU,
		       NU_SGNO_RES_PAU, NU_CEL_RES_PAU,
		       NU_VINC_PAU,
		       CalcularEdad (FE_NACI_PAC, SYSDATE, 0) AS EDAD,
			   CalcularEdad (FE_NACI_PAC, SYSDATE, 1) AS UME,
			   CASE 
					 WHEN NVL(TO_CHAR(NU_NUME_REG_PAC), 0) = 0 
					 	THEN 1 
					 ELSE ( 
					 		SELECT NU_TIAT_REG 
					 		FROM REGISTRO 
			   ...
			   END AMBITO_ATENCION,
			   CAST (NU_SEXO_PAC AS NUMBER(10)) NU_SEXO_PAC,
		       ...
		       NU_CODI_FAC_PAU, DE_NOM_FAC_PAU,
			   NU_CODI_CARR_PAU, DE_NOM_CARR_PAU,
			   CD_CODI_MUNI_PAU, CD_CODI_PAIS_PAU,
			   CD_CODI_DPTO_PAU, FECH_INI_PAU,
			    FECH_FIN_PAU, TIENE_AHC
		FROM PACIENTES
		INNER JOIN PACIENTES_ANEXO_UNAL 
		 	ON NU_HIST_PAC = NU_HIST_PAC_PAU
		WHERE UPPER(CD_CODI_EST_PAU) LIKE '%' + V_CODIGO + '%';
