CREATE OR REPLACE PROCEDURE H3i_SP_CONS_SERVS_PENDIENTES_A
 -- =============================================      
...
BEGIN
    INSERT INTO TT_RESULTADO 
		...
		LEFT JOIN Solicitudes S 
            ON HPR.NU_NUME_HPRO = S.nu_auto_soli
    ...
    WHERE PAC.NU_HIST_PAC LIKE V_NumHistoria
    	...
    	AND SER.NU_OPCI_SER IN (SELECT IDTIPO FROM TABLE(F_FILTRA_TIPOSERVICIOS (V_TipoServ)))
    ...
    OPEN CV_1 FOR
        SELECT *
        FROM TT_RESULTADO
        ORDER BY TIPOSERVICIO, FECHA_ORDEN, 
            NU_NUME_REG, NU_NUME_HICL, 
            NU_NUME_HEVO_HPRO, NU_ORDE_HPRO;
END;
