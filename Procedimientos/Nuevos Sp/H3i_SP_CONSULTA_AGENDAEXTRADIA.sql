...
BEGIN 
	...
	INSERT INTO TT_TMPAGENDADIA	
		SELECT FE_HORA_CIT, NU_HIST_PAC, 
			...
			(	SELECT CD_CODI_PLANTILLA 
				FROM R_SER_ACTI 
				WHERE R_SER_ACTI.CD_CODI_SER_RSA = CD_CODI_SER) CD_CODI_PLANTILLA 
	...
	UNION ALL
		...
		SELECT TB01.*, 
			...
			(	SELECT NO_NOMB_MED 
				FROM MEDICOS 
				WHERE CD_CODI_MED = TB02.CD_CODI_MED_EST_CIT) AS NOM_EST_CITA,
			(	SELECT NU_NUME_IND_SER 
				FROM SERVICIOS 
				WHERE CD_CODI_SER_CIT = CD_CODI_SER) AS NU_NUME_IND_SER, -- ES SERVICIO PYP
			(	SELECT CD_CODI_PLANTILLA 
				FROM R_SER_ACTI 
				WHERE R_SER_ACTI.CD_CODI_SER_RSA = CD_CODI_SER_CIT) AS CD_CODI_PLANTILLA -- PLANTILLA PYP SI FUE PARAMETRIZADA
		...
		BEGIN
			...
			INSERT INTO TT_TMPAGENDADIA2		
					SELECT	FE_HORA_CIT, NU_HIST_PAC, 
						...
						( 	SELECT NO_NOMB_MED 
							FROM MEDICOS 
							WHERE CD_CODI_MED = CITAS_MEDICAS.CD_CODI_MED_EST_CIT) AS NOM_EST_CITA,
						NU_NUME_IND_SER, 
						(	SELECT CD_CODI_PLANTILLA 
							FROM R_SER_ACTI 
				...
				UNION ALL
					...
					SELECT	TB01.*, 
						...
						(	SELECT NO_NOMB_MED 
							...
							WHERE CD_CODI_MED = TB02.CD_CODI_MED_EST_CIT) AS NOM_EST_CITA,
						(	SELECT NU_NUME_IND_SER 
							FROM SERVICIOS WHERE CD_CODI_SER_CIT = CD_CODI_SER) AS NU_NUME_IND_SER, -- ES SERVICIO PYP
						(	SELECT CD_CODI_PLANTILLA 
							FROM R_SER_ACTI 
							WHERE R_SER_ACTI.CD_CODI_SER_RSA = CD_CODI_SER_CIT) AS CD_CODI_PLANTILLA -- PLANTILLA PYP SI FUE PARAMETRIZADA
					FROM TABLE(FN_LISTACARGOSCONFIRMADOS(V_MEDICO,V_FECINICIAL,V_FECFINAL)) TB01
			...
			OPEN CV_2 FOR
				SELECT	FE_HORA_CIT, NU_HIST_PAC, 
					...
					CD_CODI_ESP_CIT, NO_NOMB_ESP,
					TX_CAUSA_INASISTENCIA,
					CAST(MIN(ISNULL(NU_NUME_LABO, 0)) AS NUMBER) AS NU_NUME_LABO,
					CD_CODI_MEDI_EST_ACDE, NOM_EST_AUT,
					CD_CODI_MED_EST_CIT, NOM_EST_CITA,
					NU_NUME_IND_SER, CD_CODI_PLANTILLA
