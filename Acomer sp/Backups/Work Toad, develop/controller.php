...
{ 	
	...
	public function actionPrueba(){
        ...
        $posiciones = $fn_mesas->arrayDatos(3); 		 

        return $this->render("prueba",["posiciones"=>$posiciones]);
	}	
    ...
    {		
        //--------------------MENUS INICIO
        //se llama la clase que llama el procedimiento y trae todos lo menus de los restaurantes
        $spMenuPlaza = new SpMenusPlaza;
        $menusPlaza = $spMenuPlaza->procedimiento();
        //se asigana las variables que retorna el procedimiento
        $categorias = $menusPlaza[0];
        $subcategorias = $menusPlaza[1];
        $terminos = $menusPlaza[2];
        $comidas = $menusPlaza[3];
        //--------------------MENUS FIN
        
        // render a la pantalla de la plaza con los parametros resectivos
        return $this->render('plaza',["categorias"=>$categorias,"subcategorias"=>$subcategorias,"terminos"=>$terminos,
            "comidas"=>$comidas]);
