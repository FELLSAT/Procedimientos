...
-- MUESTRA LOS PEDIDOS EN COCINA
	...
	(
		IN_CODIGO_COCINERO IN TAB_REL_ROL.TERCOD%TYPE,	-- CODIGO DEL COCINERO
	...
	AS
		...
		V_EMPRESA_COCINERO TAB_REL_ROL.EMPCOD%TYPE; -- EMPRESA A LA QUE PERTENECE EL COCINERO 
	...
	BEGIN
		...
		FROM TAB_REL_ROL
		WHERE TERCOD = IN_CODIGO_COCINERO;		
...
-- FACTURACION DE UNA MESA
	...
	(
		...
		IN_CELULA_MESERO IN TAB_REL_ROL.TERCOD%TYPE,    -- CEDULA DEL MESERIO
...
-- INSERCION DE LA CABECERA Y EL DETALLE DE LA FACTURA
	...
	(
		...
		IN_CODIGO_USUARIO IN TAB_REL_ROL.TERCOD%TYPE,	-- CODIGO DEL USUARIO
			...
			BEGIN
				OPEN CURSOR_PEDIDOS FOR
					SELECT DISTINCT PEDSAL ESTADO,
	                    ...
	                    PEDNUMDOC DOCUMENTO,
	                    LISTAGG(TRIM(CCOCOD),'*_') WITHIN GROUP( ORDER BY TRIM(CCOCOD),TRIM(PEDPROCOD)) PUESTO,
        				LISTAGG(TRIM(PEDPROCOD),'*_') WITHIN GROUP( ORDER BY TRIM(CCOCOD),TRIM(PEDPROCOD)) PLATO
	                FROM (
	                    SELECT VEN2.PEDSAL,
	                        VEN1.MESCOD,
	                        VEN1.PEDEMPC,
...
--	REALIZAR PEDIDOS DE UNA MESA 
	...
	(
		...
		IN_CODIGO_MESERO IN TAB_REL_ROL.TERCOD%TYPE,	 -- CODIGO DEL MESERO 
...
-- INSERTA LA CABECERA DEL PEDIDO 
	...
	(
		...
		IN_CODIGO_MESERO IN TAB_REL_ROL.TERCOD%TYPE,       -- CEDULA DEL MESERO QUE ESTA TOMANDO EL PEDIDO 
	...
	AS
		...
		V_USUARIO TAB_REL_ROL.TERCOD%TYPE;	 -- IDENTIFCARDOR (USUARIO) DEL MESERO
	...
	-- CONSULTA EL CODIGO DEL PAIS CON LA CEDULA DEL MESERO
		SELECT DISTINCT TAB_REL_ROL.EMPPAIC 
		INTO V_CODIGO_PAIS
		FROM TAB_REL_ROL 
		WHERE TAB_REL_ROL.TERCOD = IN_CODIGO_MESERO;
	...
	-- INSERCION DE LOS DATOS DEL PEDIDO CABECERA
		...
		VALUES (V_CODIGO_PAIS, IN_CODIGO_RESTAURANTE, IN_CODIGO_DOCUMENTO_PEDIDO, V_CODIGO_PEDIDO,
			...
			'0','PEDIDO','01','N',
			'0',V_CODIGO_PAIS,IN_CODIGO_RESTAURANTE,IN_CODIGO_MESERO,
	...
	BEGIN
		...
		LOOP
            ...
            WHERE A.PEDEMPC = IN_CODIGO_EMPRESA
                AND A.CCOCOD = CAST(IN_PUESTOS(I) AS CHAR(3))
...
-- HISTORIAL DE PEDIDOS EN COCINA YA HECHOS
	...
	AS
		...
		V_CODIGO_EMPRESA TAB_REL_ROL.EMPCOD%TYPE;	-- EMPRESA A LA QUE PERTENECE EL COCINERO COCINERO
	BEGIN
		-- ============================
